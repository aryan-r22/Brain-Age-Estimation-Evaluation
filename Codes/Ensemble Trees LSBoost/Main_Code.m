...
cvFolds = cvFolds( 1:n ,:);
    ...
    for  i =1:K                                 %for each fold  for i = 1 : K
        ...
        clf0 =GradientBoostingRegressor(struct('loss',loss,'n_estimators',best_n));
...
clf1=GradientBoostingRegressor(struct('loss',loss,'n_estimators',best_n));
...
plot(qw,qe,'--k')
fm = gcf;
exportgraphics(fm,'Ensemble_Trees_LSBoost.png','Resolution',300)
