...
for  i =1:K                                 %for each fold  for i = 1 : K
    ...
    ridge = Ridge(struct('alpha',best_ap));
...
feature_map1 = Nystroem(struct('kernel',kernel,...
    'n_components',n_components));
ridge1 = Ridge(struct('alpha',best_ap));
clf1 = make_pipeline(feature_map1,ridge1);
...
plot(qw,qe,'--k')
fm = gcf;
exportgraphics(fm,'Bag.png','Resolution',300)
