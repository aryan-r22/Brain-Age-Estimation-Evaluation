...
for  i =1:K                                 %for each fold  for i = 1 : K
    ...
    %Mdl = fitrlinear(DataTrain,AgeTrain); % 'Regularization','ridge' or 'lasso';
    Mdl = fitrgp(DataTrain, AgeTrain, 'KernelFunction', 'rationalquadratic');
...
%Mdl_onTest_HC = fitrlinear(MainData,Age_HC_Train_main, 'Regularization','lasso'); % 'Regularization','ridge' or 'lasso';
Mdl_onTest_HC = fitrgp(MainData, Age_HC_Train_main, 'KernelFunction',  'rationalquadratic');
...
%Mdl_onTest_MCI = fitrlinear(MainData, Age_HC_Train_main, 'Regularization','lasso');
Mdl_onTest_MCI = fitrgp(MainData,Age_HC_Train_main, 'KernelFunction', 'rationalquadratic');
...
%Mdl_onTest_AD = fitrlinear(MainData,Age_HC_Train_main, 'Regularization','lasso'); % 'Regularization','ridge' or 'lasso';
Mdl_onTest_AD = fitrgp(MainData,Age_HC_Train_main, 'KernelFunction', 'rationalquadratic');
...
hold on
plot( Test_Age_AD,PredictTest_AD, 'go','DisplayName','MCI' )
...
yline = coeff(1)*xline+coeff(2);
hold on
plot(xline,yline,'g-','DisplayName','MCI')
legend('Location','northwest','AutoUpdate','off');
qw=1:1:100;
qe=1:1:100;
plot(qw,qe,'--k')
fm = gcf;
exportgraphics(fm,'Gaussian_Regression_RationalQuadratic.png','Resolution',300)
hold on
