...
cvFolds = cvFolds( 1:n ,:);
    ...
    Mdl = fitrsvm(DataTrain,AgeTrain,'Standardize',false,'KernelFunction','gaussian','OutlierFraction',0.0,'OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('KFold',10)); % ,'KernelScale','auto' . false
...
Mdl_onTest_HC = fitrsvm(MainData,Age_HC_Train_main,'Standardize',false,'KernelFunction','gaussian','OutlierFraction',0.0,'OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('KFold',10)); % ,'KernelScale','auto'
...
%Mdl_onTest_MCI = fitrsvm(MainData,Age_HC_Train_main,'Standardize',false,'KernelFunction','gaussian','OutlierFraction',0.0,'OptimizeHyperparameters','auto'); % ,'KernelScale','auto'
%Mdl_onTest_MCI = fitrlinear(MainData,Age_HC_Train_main, 'Regularization','ridge');

PredictTest_MCI_Before= predict(Mdl_onTest_HC,MainTest_MCI);
...
%
%Mdl_onTest_AD = fitrsvm(MainData,Age_HC_Train_main,'Standardize',false,'KernelFunction','gaussian','OutlierFraction',0.0,'OptimizeHyperparameters','auto'); % ,'KernelScale','auto'
%Mdl_onTest_AD = fitrlinear(MainData,Age_HC_Train_main,'Regularization','ridge'); % 'Regularization','ridge' or 'lasso';

PredictTest_AD_Before= predict(Mdl_onTest_HC,MainTest_AD);
...
plot(qw,qe,'--k')
fm = gcf;
exportgraphics(fm,'Gaussian_SVR.png','Resolution',300)
hold on
