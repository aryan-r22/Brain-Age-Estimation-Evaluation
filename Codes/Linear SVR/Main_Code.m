...
for  i =1:K                                 %for each fold  for i = 1 : K
    ...
    Mdl = fitrsvm(DataTrain,AgeTrain); % ,'KernelScale','auto' . false
...
Mdl_onTest_HC = fitrsvm(MainData,Age_HC_Train_main); % ,'KernelScale','auto'
...
Mdl_onTest_MCI = fitrsvm(MainData,Age_HC_Train_main); % ,'KernelScale','auto'
...
%
Mdl_onTest_AD = fitrsvm(MainData,Age_HC_Train_main); % ,'KernelScale','auto'
...
plot(qw,qe,'--k')
fm = gcf;
exportgraphics(fm,'Linear_SVR.png','Resolution',300)
hold on
