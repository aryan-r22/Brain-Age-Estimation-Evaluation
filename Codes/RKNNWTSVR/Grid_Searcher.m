...
range_t2=[10^-5,10^-4,10^-3,10^-2,10^-1,10^0,10^1,10^2,10^3,10^4,10^5];
count=1;
cnt=1;
...
for t1=range_t1
    for t2=range_t2
        ...
        for  i =1:K                                 %for each fold  for i = 1 : K
            ...
            %test_train_regression (train_data,test_data,c2,u1,u2,D)
            %function[u1,u2,D,time]=lsvm(train_data,ep1,ep2,c1,c2,itmax,tol)
            [u1,u2,D,~]=lsvm([DataTrain AgeTrain],0.01,0.01,t1,0.9,t2,100,1e-5);
            test=test_train_regression([DataTrain AgeTrain],MainTestData ,t1,0.9,u1,u2,D);
...
save('best_params.mat','best_t1','best_t2');