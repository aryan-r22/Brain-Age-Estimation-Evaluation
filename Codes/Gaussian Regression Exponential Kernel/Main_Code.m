...
for  i =1:K                                 %for each fold  for i = 1 : K
    ...
    Mdl = fitrgp(DataTrain, AgeTrain, 'KernelFunction', 'exponential');
...
Mdl_onTest_HC = fitrgp(MainData, Age_HC_Train_main, 'KernelFunction',  'exponential');
...
Mdl_onTest_MCI = fitrgp(MainData,Age_HC_Train_main, 'KernelFunction', 'exponential');
...
Mdl_onTest_AD = fitrgp(MainData,Age_HC_Train_main, 'KernelFunction', 'exponential');
...
plot(qw,qe,'--k')
fm = gcf;
exportgraphics(fm,'Gaussian_Regression_Exponential.png','Resolution',300)
hold on
