...
for  i =1:K                                 %for each fold  for i = 1 : K
    ...
    dnne = newdnne(best_enSize, best_baSize, DataTrain, AgeTrain, best_reg);
...
dnne1 = newdnne(best_enSize, best_baSize, MainData, Age_HC_Train_main, best_reg);
    [dnne1, rmse1] = traindnne(dnne1,MainData, Age_HC_Train_main );
    
PredictTest_HC_Before= simdnne(dnne1,MainTest_HC);
...
PredictTest_MCI_Before= simdnne(dnne1,MainTest_MCI);
...
PredictTest_AD_Before= simdnne(dnne1,MainTest_AD);
...
plot(qw,qe,'--k')
fm = gcf;
exportgraphics(fm,'DNNE.png','Resolution',300)
hold on
...
DNNE = cell(length(Age_HC_Test_main),6);
DNNE(1:88,1) = num2cell(Age_HC_Test_main);
DNNE(1:88,2) = num2cell(PredictTest_HC);
DNNE(1:70,3) = num2cell(Test_Age_MCI);
DNNE(1:70,4) = num2cell(PredictTest_MCI);
DNNE(1:30,5) = num2cell(Test_Age_AD);
DNNE(1:30,6) = num2cell(PredictTest_AD);
