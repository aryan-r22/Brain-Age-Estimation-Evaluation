...
cvFolds = cvFolds( 1:n ,:);
    ...
    for  i =1:K                                 %for each fold  for i = 1 : K
        ...
        metric = 'euclidean';
        weights = {'distance'};
        mdl = kNNeighborsRegressor(best_k,metric,weights(1));
...
metric1 = 'euclidean';
weights1 = {'distance'};
 mdl1 = kNNeighborsRegressor(best_k,metric1,weights1(1));
 mdl1 = mdl1.fit(MainData,Age_HC_Train_main);
 PredictTest_HC_Before=mdl1.predict(MainTest_HC)';
end
%% Bias adjustment
PredictTest_HC=[];
...
PredictTest_MCI_Before= mdl1.predict(MainTest_MCI)';
...
PredictTest_AD_Before= mdl1.predict(MainTest_AD)';
...
plot(qw,qe,'--k')
fm = gcf;
exportgraphics(fm,'KNN.png','Resolution',300)
