...
cvFolds = cvFolds( 1:n ,:);
    ...
    for  i =1:K                                 %for each fold  for i = 1 : K
        ...
        mdl = fitrnet(DataTrain,AgeTrain,"LayerSizes",repmat(best_k1,1,best_n),'Activations',["sigmoid"]);
...
%Mdl_onTest_HC = fitrlinear(MainData,Age_HC_Train_main,'Regularization','ridge'); % 'Regularization','ridge' or 'lasso';
 mdl1 = fitrnet(MainData,Age_HC_Train_main,"LayerSizes",repmat(best_k1,1,best_n),'Activations',["sigmoid"]);
        
 
 PredictTest_HC_Before=predict(mdl1,MainTest_HC);
end
%% Bias adjustment
PredictTest_HC=[];
...
PredictTest_MCI_Before= predict(mdl1,MainTest_MCI);
...
PredictTest_AD_Before= predict(mdl1,MainTest_AD);
...
plot(qw,qe,'--k')
fm = gcf;
exportgraphics(fm,'NN.png','Resolution',300)
